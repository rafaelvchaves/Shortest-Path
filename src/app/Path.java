...
public class Path {
    ...
    /** Return the length of the shortest path from node v to node end <br>
     * ---or -1 if a path does not exist. */
    public static int shortestDistance(Node v, Node end) {
        Heap<Node> F= new Heap<>(false);
        /** map contains an entry for each node in F or S. <br>
         * ... Thus, |map| = |F| + |S|.<br>
         * ... For each such node, map contains the shortest known distance<br>
        ...
        while (F.size() != 0) {
            Node f= F.poll();
            if (f == end) return map.get(end);
            int fDist= map.get(f);

            for (Edge e : f.getExits()) {// for each neighbor w of f
                Node w= e.getOther(f);
                int newWdist= fDist + e.length;
                Integer wDB= map.get(w);
                if (wDB == null) { // if w not in F or S
                    map.put(w, newWdist);
                    F.add(w, newWdist);
                } else if (newWdist < wDB) {
                    map.put(w, newWdist);
                    F.updatePriority(w, newWdist);
                }
            }
